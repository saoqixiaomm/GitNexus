...
const SCALA_EXTENSIONS: readonly string[] = ['.scala', '.sc'];
...
/**
 * Scala JVM resolution strategy — wildcard/member imports with Java interop.
 *
 * Handles Scala 2 `._` after provider preprocessing to `.*`, Scala 3 `*`,
 * member imports such as `com.example.Constants.VALUE`, and common top-level
 * function imports by falling back to package-directory scans for lowercase
 * last segments.
 */
